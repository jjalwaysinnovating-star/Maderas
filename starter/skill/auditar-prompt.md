...
2. Lee (solo lectura) todo lo que ve el modelo: el prompt y las tools.
   ```
   wrangler d1 execute DB --remote --json --command "SELECT key, value FROM settings WHERE key IN ('custom_instructions','system_prompt_override','business_context','brand_voice');"
   ```
   Y las tools activas + sus descripciones en `src/tools/` (para juzgar si el prompt las aprovecha).
...
7. **Mantenibilidad** — ¿datos volátiles (precios, promos) hardcodeados que deberían ir a la KB?
