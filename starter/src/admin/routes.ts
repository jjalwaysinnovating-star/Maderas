...
const AUTH_LIBRE = ["/login", "/logout", "/invitacion", "/recuperar", "/restablecer"];
adminApp.use("*", async (c, next) => {
  ...
  // Sin credenciales: si el bot ya tiene usuarios de panel, el navegador va al
  // login bonito; scripts/curl (y bots sin usuarios) siguen con Basic.
  const { hayPanelUsers } = await import("./equipo");
  const hay = await hayPanelUsers(new Db(c.env.DB)).catch(() => false);
  const esNavegador = (c.req.header("Accept") ?? "").includes("text/html");
  if (hay && esNavegador && c.req.method === "GET") return c.redirect("/admin/login");
...
adminApp.get("/login", async (c) => {
  ...
  return c.html(renderLogin(c.env, { copy: await copyLogin(c.env) }));
});
...
adminApp.post("/login", async (c) => {
  ...
  if (uid === "bloqueado") return c.html(renderLogin(c.env, { error: t("equipo.errorBloqueado"), copy: await copyLogin(c.env) }), 429);
  if (!uid) return c.html(renderLogin(c.env, { error: t("equipo.errorLogin"), copy: await copyLogin(c.env) }), 401);
...
adminApp.post("/perfil", async (c) => {
  ...
  const r = await eq.updateProfile(db, e.PANEL_UID, {
    ...
    puesto: String(body.puesto ?? ""), horario: String(body.horario ?? ""),
...
adminApp.post("/recuperar", async (c) => {
  ...
  if (r) {
    ...
    // Sin proveedor (o si falló): el admin recibe el link como ticket y lo pasa a mano.
    if (!mailerProvider(c.env)) {
      try {
        const { createHandoffTicket } = await import("../tools/handoffHuman");
        await createHandoffTicket(c.env, {
        ...
        await eq.audit(db, { id: r.id, label: email }, "reset_solicitado", "sin correo → ticket al admin");
      } catch (e) { console.warn("[equipo] ticket de reset falló:", e); }
    }
  }
  const { mailerProvider: mp } = await import("../mailer");
  return c.html(renderRecuperar(c.env, { enviado: true, conCorreo: Boolean(mp(c.env)) }));
});
