...
export function renderLogin(env: Env, opts: { error?: string; copy?: LoginCopy } = {}): string {
  const t = traductor(env);
  const b = resolveBranding(env);
  ...
  return paginaSuelta(env, c.titulo, `
    ...
    <form method="POST" action="/admin/login">
      <label class="campo"><input type="email" name="email" placeholder=" " autocomplete="username"><span>${esc(t("equipo.correo"))}</span></label>
  ...
  return paginaSuelta(env, t("equipo.inviteTitulo"), `
    ...
    <form method="POST" action="/admin/invitacion/${esc(token)}">
      ...
      <div class="grupo">
        ...
        <label class="campo" style="margin-top:8px"><input type="text" name="horario" placeholder=" " pattern="\\d{1,2}:\\d{2}\\s*-\\s*\\d{1,2}:\\d{2}" title="09:00-18:00"><span>${esc(t("equipo.horario"))}</span></label>
...
export function renderRecuperar(env: Env, opts: { enviado?: boolean; conCorreo?: boolean }): string {
  const t = traductor(env);
  const b = resolveBranding(env);
  const cuerpo = opts.enviado
    ? `<h1>${esc(t("equipo.recuperarTitulo"))}</h1>
       <div class="ok">${esc(opts.conCorreo ? t("equipo.recuperarEnviado") : t("equipo.recuperarSinCorreo"))}</div>
       <p class="nota"><a class="link" href="/admin/login">${esc(t("equipo.volverLogin"))}</a></p>`
    : `<h1>${esc(t("equipo.recuperarTitulo"))}</h1>
...
function camposHorario(t: ReturnType<typeof traductor>, u: { horario: string | null; dias: string | null; puesto: string | null }, INP: string): string {
  ...
  return `
    ...
    <label style="display:block;margin-bottom:8px"><div style="font-size:11.5px;font-weight:700;letter-spacing:.04em;text-transform:uppercase;color:var(--dim);margin-bottom:5px">${esc(t("equipo.horario"))}</div>
      <input name="horario" type="text" value="${esc(u.horario ?? "")}" placeholder="09:00-18:00" style="${INP}"></label>
...
export function renderEquipo(
  ...
  const body = `
    <p style="max-width:640px;color:var(--dim);font-size:13.5px;line-height:1.6;margin-bottom:18px">${esc(t("equipo.intro"))} <a href="/admin/equipo/bitacora" style="color:var(--accent)">${esc(t("equipo.bitacora"))} →</a></p>
    ${opts.error ? `<div style="background:#fbe9e4;border:1.5px solid #d99a83;padding:10px 14px;font-size:13px;margin-bottom:14px">${esc(opts.error)}</div>` : ""}
    ${opts.inviteUrl ? `
