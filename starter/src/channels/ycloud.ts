...
export async function parseYCloudEvents(
  ...
  if (m.type === "text") {
    ...
    return []; // video/document/location/… no soportados aún
  }
...
export const ycloudAdapter: ChannelAdapter = {
  ...
  async sendReply(reply: OutgoingReply, env: Env): Promise<void> {
    ...
    for (let i = 0; i < reply.chunks.length; i++) {
      ...
      // logéalo con el cuerpo para ver el motivo exacto, no lo tragues en silencio.
      if (!res.ok) {
    ...
    for (const m of reply.media ?? []) {
      ...
      if (!res.ok) {
        const errBody = await res.text().catch(() => "");
        console.error(`ycloud media send ${res.status}: ${errBody}`);
      }
    }
  },
};
