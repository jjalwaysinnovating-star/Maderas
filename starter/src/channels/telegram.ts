...
export const telegramAdapter: ChannelAdapter = {
  ...
  async sendReply(reply: OutgoingReply, env: Env): Promise<void> {
    ...
    for (const m of reply.media ?? []) {
      ...
      const method =
        m.kind === "image" ? "sendPhoto" : m.kind === "video" ? "sendVideo" : m.voice ? "sendVoice" : "sendAudio";
      const field = m.kind === "image" ? "photo" : m.kind === "video" ? "video" : m.voice ? "voice" : "audio";
      ...
      }).catch((e) => console.error("[telegram] media send:", e));
    }
  },
