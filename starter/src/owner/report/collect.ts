...
async function baseStats(db: Db, since: number, until: number): Promise<ReportStats> {
  ...
  const [customerMessages, newConversations, newLeads, hotLeads, ticketsOpened, ticketsResolved, upsetCustomers] =
    await Promise.all([
      c("SELECT COUNT(*) AS n FROM messages WHERE created_at > ? AND created_at <= ? AND role = 'user'"),
      c("SELECT COUNT(*) AS n FROM conversations WHERE started_at > ? AND started_at <= ?"),
      c("SELECT COUNT(*) AS n FROM leads WHERE created_at > ? AND created_at <= ?"),
      c("SELECT COUNT(*) AS n FROM conversation_insights WHERE analyzed_at > ? AND analyzed_at <= ? AND sale_opportunity = 1"),
      c("SELECT COUNT(*) AS n FROM tickets WHERE created_at > ? AND created_at <= ?"),
      c("SELECT COUNT(*) AS n FROM tickets WHERE resolved_at > ? AND resolved_at <= ?"),
...
export async function collectReportContext(env: Env, now: number): Promise<ReportContext> {
  ...
  const [msgRows, followupsSent, reviewsRequested, trendRows] = await Promise.all([
    db.all<{ created_at: number }>(
      "SELECT created_at FROM messages WHERE created_at > ? AND role = 'user'",
      [since],
    ),
    ...
    db.all<{ d: number; n: number }>(
      "SELECT CAST((? - created_at) / 86400000 AS INTEGER) AS d, COUNT(*) AS n FROM messages WHERE created_at > ? AND role = 'user' GROUP BY d",
      [now, now - 7 * DAY_MS],
    ),
  ]);
