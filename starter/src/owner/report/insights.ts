 ...
 * (createModel, el que el dueño ya configuró) para leer los datos del día y
 * escribir un análisis de negocio: resumen, hallazgos y acciones para mañana.
 * Grounded: solo razona sobre los datos que se le pasan, no inventa. Si el
 * modelo falla o devuelve basura, cae a un resumen mínimo con los números.
 */
...
Devuelve SOLO un JSON válido, sin texto alrededor, con esta forma:
{
  ...
  try {
    const { model } = createModel(env, "smart", await loadLlmOverrides(env));
